...
router.get("/all", function(req, res) {
  req.Registerees.find({}, 'registereeId name steps -_id', {sort:{steps:-1}},function(err, registerees) {
    if (err) {
      res.send(err);
    }
    else {
...
router.get("/top/:number", function(req, res) {
  req.Registerees.find({}, 'registereeId name steps png -_id',{limit:parseInt(req.params.number),sort:{steps:-1}}, function(err, registerees) {
    if (err) {
      res.send(err);
    }
    else {
...
router.get("/position/steps/:userSteps", function(req, res) {
  req.Registerees.count({steps:{$gt:parseInt(req.params.userSteps)}}, function(err, position) {
    if (err) {
      res.send(err);
    }
    else {
...
router.get("/position/user/:registereeId", function(req, res) {
  req.Registerees.findOne(req.params, function(err, registeree) {
    if (err){
      ...
      req.Registerees.count({steps:{$gt:parseInt(registeree.steps)}}, function(err, position) {
        if (err) {
          res.send(err)
        } else {
          req.Registerees.count(function(err, count) {
            if (err) {
              res.send(err);
            }
            else {
